<script lang="ts">
    ...
    async function handleSubmit() {
        await Promise.all(selectedSessions.map((session) => {
            // @ts-ignore
            return $api.session.apiSessionsIdDelete(session.id);
        }));
...
<Form on:submit={handleSubmit}>
    <FormGroup>
        ...
        <p>
            Al borrar una sesión será como si hubieras cerrado sesión en el agente
            con el que la iniciaste.
        </p>
    </FormGroup>
